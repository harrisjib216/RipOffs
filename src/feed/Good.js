...
import { StyleSheet, View, Text, TextInput, TouchableOpacity, Dimensions, FlatList, Image, ScrollView, WebView } from 'react-native';
import * as firebase from "firebase";
import { _getGood } from './../components/loadData';
...
class Good extends Component {
  ...
  render() {
    return (
      <View style={styles.container}>
        {/* nav bar */}
        <Navbar navtitle={'GOOD DEALS'} underlineWidth={150}/>

        
        {/* list of posts */}
        <FlatList
          data={this.state.goodDeals}
        ...
        {/* nav tab bar */}
        <TabBar navigator={this.props.navigation} currentScreen={'Good'} />
      </View>
    );
  }
  ...
  componentWillMount() {
    firebase.auth().onAuthStateChanged((user) => {
      this.setState({goodDeals: _getGood(), myUID: user.uid});
    });
  }
    ...
    return (
      <View style={styles.postContainer}>
        ...
        <View style={{flexDirection: 'row', marginTop: 5}}>
          ...
          <Text style={styles.goodBG}>good</Text>
        </View>
